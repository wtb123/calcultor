...
use common\models\AuthAssignament;
/**
...
class AdminuserController extends Controller
{
    ...
    public function actionPrivilege($id)
    {
        ...
        //step2.当前用户的权限
        $AuthAssignments=AuthAssignament::find()->select(['item_name'])
        ...
        {
            AuthAssignament::deleteAll('user_id=:id',[':id'=>$id]);
            $newPri=$_POST['newPri'];
            $arrlength=count($newPri);
            for($x=0;$x<$arrlength;$x++)
            {
                $aPri=new AuthAssignament();
